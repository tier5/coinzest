			...
			<script type="text/javascript">
				...
				jQuery(document).ready(function($) {
					...
					if (shiny_updates) {
						...
						jQuery(document).on('wp-plugin-update-success wp-theme-update-success', function() {
...
// 						});
					...
					function updates_intercept(e, passthis, checklink, via_shiny_updates, type) {
						...
						type = ('undefined' === typeof type) ? 'plugin' : 'theme';
...
// 							link = '';
					...
					$bulk_action_form.on( 'submit', function( e ) {
						...
						if ( jQuery( '#bulk-action-selector-top' ).val() == 'update-selected' ) {
							...
							something_happening = false;
							if (shiny_updates) { wp.updates.<?php echo $lock_variable; ?> = false; }
						}
					} );
...
// 					jQuery('body.themes-php .theme-browser .update-message').on('click', function(e) {
// 						if (updraft_actually_proceeding) {
// 							updates_intercept(e, this, true, shiny_updates, 'theme');
// 						} else {
// 							console.log("UpdraftPlus: wp-theme-updating event triggered: intercepting");
// 							updates_intercept(e, this, false, shiny_updates, 'theme');
// 						}
// 					});

					jQuery(document).on('wp-theme-updating', function(e) {
// 					console.log("wp-theme-updating caught");
// 					return;
						// 	https://core.trac.wordpress.org/ticket/37216#comment:6
						if (updraft_actually_proceeding) {
							console.log("UpdraftPlus: wp-theme-updating event triggered: letting pass (already handled)");
						} else {
							console.log("UpdraftPlus: wp-theme-updating event triggered: intercepting");
							updates_intercept(e, this, false, shiny_updates, 'theme');
						}
