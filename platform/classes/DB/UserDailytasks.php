...
class DB_UserDailyTasks extends CI_Model {
	...
	function is_user_have_active_tasks_for_current_round($user_id) {
		/*
		$arr_criteria = Array();
		...
		*/
		$this->db->select(DB_UserDailyTasks::$TBLE_NAME.".id")->from(DB_UserDailyTasks::$TBLE_NAME);
		$this->db->sele('user_id',$user_id);
		$this->db->where('user_id',$user_id);
		//$this->db->where('round_id',$round_id);
		$this->db->where('is_user_completed',0);
		$this->db->where('status_id',self::$ACTIVE_STATUS_ID);
	...
	*/
	...
	function get_user_number_of_active_tasks_for_current_round($user_id) {
		...
		*/
		...
		$this->db->select("count(".DB_UserDailyTasks::$TBLE_NAME.".id) as total")->from(DB_UserDailyTasks::$TBLE_NAME);
		$this->db->where('user_id',$user_id);
		$this->db->where('is_user_completed',0);
